...
def get_ann_path(data_dir: str,
    ...
    assert dataset in ['', 'lsp', 'mpii']  # '' is coco
    assert data_type in ['person_keypoints', 'captions', 'instances', 'image_info']
    assert division in ['train', 'val', 'test', 'demo']
    path = '{}/annotations/{}_{}_{}{:d}.json'.format(data_dir, dataset, data_type, division, year)
...
def gen_record(data_dir,
    ...
    for img, img_id in zip(imgs, img_ids):
        ...
        if len(anns) == 0:
            continue
        yield os.path.join(data_dir, img['file_name']), anns
...
if __name__ == '__main__':
    i = 0
    for img_path, annotations in gen_record("D:/GeekGank/workspace/Data/HUMAN/mpii_human_pose_v1_u12_2",
                                            data_type='person_keypoints', year=2014,
                                            division='train', dataset='mpii'):
        print(img_path)
        print(annotations)
        i += 1
        if i > 10:
